...
ggplot(
  ...
  geom_vline(
    xintercept = c(1968, 1969),  # Add key years for transitions
    linetype = "dashed",
    color = "gray",
    size = 0.8
  ) +
  annotate(
    "text", x = 1968.5, y = max(pink_floyd_albums_tidy$value), 
    label = "Syd Barrett Departs", color = "gray", angle = 90, vjust = -0.5
  ) +
  labs(
    title = "Pink Floyd's Stats Over Time",
    x = "Release Year",
...
ggplot(
  ...
  annotate(
    ...
    xmin = 1967, xmax = 1969,
    ymin = -Inf, ymax = Inf,
    alpha = 0.2, fill = "#e7298a"  
  ) +
  annotate(
    "rect",
    xmin = 1969, xmax = 1983,
    ymin = -Inf, ymax = Inf,
    alpha = 0.2, fill = "#e6ab02"
  ) +
  labs(
    title = "Pink Floyd's Stats Over Time",
    x = "Release Year",
